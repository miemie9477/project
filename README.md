...
* **商品圖的檔名用商品編號**
* 商品頁面Dropdown要記得做(品牌、國家)
(懶得做可以先註解掉，先不要直接刪掉)
* 搜尋欄執行可以回傳結果，但request的值我是在程式裡宣告，不是從搜尋欄的textfield抓
    * 要從搜尋欄裡取值並丟到上面的keyword
    * 把搜尋結果顯示出來
* 商品詳細頁面
    * 商品編號在 *Gordon'sGin.js* (組合元件的頁面)，做其他檔案的時候那邊傳入的商品編號(pNo)要記得改
    * 已經回傳商品詳細資訊了，把內容貼到畫面上
    * 商品留言板要做，每輸出5筆留言就換頁
    * 商品加入購物車已完成
* 購物車
    * 購物車資料已經回傳
    * 我忘記把購物車每項物品的總金額寫上去了，幫我補一下
    * 旁邊金額要隨 *購物車已勾選的商品* 金額 *加總*
    * 上面全選要做
    * 把已勾選的物品傳到結帳頁面(結帳金額也要記得改)，後端才能接資料串API
* admin
    * 三個資料分三個頁籤
    * 商品資料已經回傳
    * 交易資料目前資料庫沒東西無法測試，所以API還沒寫，我明天再匯幾筆
* 會員修改資料幫我貼到下面修改區(設成default value)
* 做好詳細頁面幫我把Homepage三張廣告的按鈕導到相對應的頁面
